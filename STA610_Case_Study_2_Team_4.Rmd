...
knitr::opts_chunk$set(echo = FALSE, cache=TRUE, message = FALSE, warning = FALSE, fig.width = 6, fig.height = 3)
```

```{r, echo=FALSE, warning=FALSE, message=FALSE, cache=FALSE}
if (!require(pacman)) install.packages("pacman")
pacman::p_load(tidyverse, lme4, gridExtra, grid, ggplot2, lattice, redres, stringr, influence.ME, knitr, GGally, corrplot, brms, rstan, cowplot, directlabels, tidybayes, parallel, ROCR, bayesplot)
...
kable(post_summary_fixed, digits = 3, align = 'c')
```
...
A coefficient with no effect (estimate of 0 on log-odds) will have an inverse logit of 0.5. Negative and positive effects will produce inverse logit results of less than 0.5 and greater than 0.5 respectively. The difference from 0.5 can interpreted as the expected decrease / increase in the probability of voting from the baseline.
...
Compared to an 18-25 year CST voter. All age effects are significant, and we observe an increase of voter turnout with age (again, for CST party only).
...
Female turnout was higher in the DEM, LIB, REP, and UNA parties, with differences ranging from 0.03927877 - 0.03823569 = 0.00104308 higher in the LIB party to 0.01381628 higher in the DEM party. Male turnout was higher in the CST and GRE parties, with the GRE party having the bigger difference at 0.04917241 - 0.03308598 = 0.01608643.
...
For people who voted for the Democrats, the turnout rate for the registered voters who ages range from 18 to 25 is 6.85%. The turnout rate for the registered voters whose ages range from 26 to 40 is 7.85%. The turnout rate for the registered voters whose ages range from 41 to 65 is 17.80%. The turnout rate for the registered voters whose ages are over 66 is 17.91%.

For people who voted for the Republicans, the turnout rate for the registered voters who ages range from 18 to 25 is 8.46%. The turnout rate for the registered voters whose ages range from 26 to 40 is 10.48%. The turnout rate for the registered voters whose ages range from 41 to 65 is 20.07%. The turnout rate for the registered voters whose ages are over 66 is 19.99%.

For people who voted for the Libertarians, the turnout rate for the registered voters who ages range from 18 to 25 is 3.93%. The turnout rate for the registered voters whose ages range from 26 to 40 is 5.26%. The turnout rate for the registered voters whose ages range from 41 to 65 is 7.72%. The turnout rate for the registered voters whose ages are over 66 is 8.42%.
