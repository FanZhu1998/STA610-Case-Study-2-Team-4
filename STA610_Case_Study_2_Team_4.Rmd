...
logit(\pi_{ij}) = \beta_0 + \beta_{1a} I(S_i = a) + \beta_{2b} I(A_i = b) + \beta_{3c} I(P_i = c) + \beta_{4bc} I( A_i = b, P_i = c) + \beta_{5ab} (S_i = a, A_i = b) + b_{0j} \\
b_{0j} \sim N(0, \tau^2) \\
p(\beta_0) \sim N(0, 10) \\
p(\beta) \sim MVN(0, 10 * I_{n\beta}) \\
p(\tau^2)  \sim \text{Student-t}(\nu=3, \mu=0, \sigma=1)
$$
...
```{r Coefficient Table}
post_summary <- default_model %>% posterior_summary()
...
Note: Effect for race is **not significant**. Ethnicity **does** appear to be significant

It is easy to see the effect of race/ethnicity. The default level for race is ... and the default level for ethnicity is ... Because 

Interpreting age (be aware of interaction - we cannot say "holding all else constant" because the age effect has an interaction that *depends* on party)

Interpreting sex (be aware of interaction - we cannot say "holding all else constant" because the age effect has an interaction that *depends* on party)

Interpreting party (be aware of interaction - we cannot say "holding all else constant" because the age effect has an interaction that *depends* on age/sex)
...
For each *party*, investigate turnout rate (obtained via `inv_logit_scaled`) between females and males. This includes calculating the appropriate combination of main effects + interaction, and then converting to turnout
