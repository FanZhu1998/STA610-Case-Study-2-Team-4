...
Some NA's get introduced on the total voters here. Could be a consequence of the full join. But I'm not sure why `total_registered_voter` would have NA's. In any case, it's only 81 - pretty trivial. 

The other (`total_voted_voter`) is more substantial. This is potentially due to when none of the registered voters for a given group actually voted. Thus, the full join would have produced NA's here. Let's investigate:

```{r}
# Hypothesis - this should produce no rows
missing_voters <- df %>% filter(is.na(total_voted_voter))
inner_join(missing_voters, VS_sampled, 
           by=c('County' = 'County', 
                'Precinct' = 'Precinct', 
                'Vtd' = 'Vtd', 
                'Age' = 'Age', 
...
We have a problem - `total_voter` has combinations appearing that show up as NA for `total_voted_voter` in `df`. Let's see an example:

```{r}
# Bug
VS_sampled %>% filter(County == 'NASH' & Vtd == '0006' & Precinct == 'P03A' & Age == 'Age 26 - 40' & Party == 'DEM' & Race == 'A')
df %>% filter(County == 'NASH' & Vtd == '0006' & Precinct == 'P03A' & Age == 'Age 26 - 40' & Party == 'DEM' & Race == 'A')
```
...
```{r}
df %>% filter(total_registered_voter < total_voted_voter)
```

At least it's only 73 rows
...
df_plot %>% 
    ...
    ggplot(aes(x=Race, y=prop, fill=Race)) + geom_col()

df_plot %>% 
    group_by(Age) %>% 
    summarise(prop=sum(total_voted_voter) / sum(total_registered_voter)) %>% 
    ggplot(aes(x=Age, y=prop, fill=Age)) + geom_col()
